...
NEW_DB_USER=sandboxreader
DC_SCHEMA=agdc

echo "Creating Sandbox Reader login role"
createuser -h "$DB_HOSTNAME" -p "$DB_PORT" -U "$ADMIN_USER" "$NEW_DB_USER" || true

echo "Resetting Sandbox Reader user password"
...
echo "Giving read access to agdc schema to Sandbox Reader"
...
echo "Adding Sandbox Reader user credentials to param-store"
aws ssm put-parameter --region "${REGION}" \
  --name "/${CLUSTER_ID}/sandbox_reader/db.creds" \
  --value "${NEW_DB_USER}:${random}" \
  --description "Sandbox Reader user credentials" \
